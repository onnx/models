...
* 5 required files under models/python/XXX/ directory:
  ...
  - `test_data_set_0`: a directory containing the test data set. The test data set should be in the format of .pb. The test data set can be generated by the script: https://github.com/onnx/models/blob/main/workflow_scripts/ort_test_dir_utils.py. For details, please check the below section: [test data creation](#test-data-creation).
...
1. Update models/python/model-metadata.yaml with the new model information.
2. Update ONNX_HUB_MANIFEST.json for ONNX Hub: If this PR does update/add .onnx, please use `python workflow_scripts/generate_onnx_hub_manifest.py --target diff` to update ONNX_HUB_MANIFEST.json with according model information (especially SHA) for ONNX Hub.
