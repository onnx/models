...
* 5 required files under models/python/XXX/ directory:
  - `XXX.onnx`: the single ONNX model file with the same name as the directory. Please use opset version as model's postfix. For example, `vgg19-18.onnx` is the ONNX model file for VGG19 model with opset 18.
  - `model.py`: a Python script named model.py for converting ONNX models from PyTorch, TensorFlow, Huggingface or etc. The model.py should be able to convert the model from source framework to ONNX format with the specified name as above. The script should be able to run with the provided requirements.txt and therefore people can reproduce the ONNX model with the script.
  - `requirements.txt`: a text file listing all the required Python packages and their versions.
  - `test_data_set_0`: a directory containing the test data set. The test data set should be in the format of .pb. The test data set can be generated by the script: https://github.com/onnx/models/blob/main/workflow_scripts/ort_test_dir_utils.py.
  - README.md: a readme file describing the model and how to use the model.
* Update models/python/model-metadata.yaml with the new model information.
* The ONNX file needs to pass onnx.checker with full_check and the test data set needs to be validated by onnxruntime from CI pipelines.
